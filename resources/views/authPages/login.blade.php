...
<div class="flex justify-center items-center min-h-full w-full mt-24">
    <form action="{{ route('login') }}" method="post" class="flex flex-col w-1/4 space-y-4">
        ...
        @csrf
        <input class="border p-4" type="text" name="name" required autofocus value="{{old ('name')}}" placeholder="username">
        ...
        <a href="{{route('githublogin')}}" class="bg-gray-300 rounded-md py-3">Login With Github</a>
        <a href="{{route('googlelogin')}}" class="bg-blue-300 rounded-md py-3">Login With Google</a>
